...
;(function (global, factory) {
  ...
  async function eonitem (__eo = {}) {
    ...
    // git clone --depth=50 --branch=master https://github.com/gatsbyjs/gatsby-docker.git 
    ...
    // $ export MAIN_TAG=`if [ "$TRAVIS_BRANCH" = "master" ]; then echo "latest"; else echo "$MAIN_TAG-onbuild" ; fi`
    // $ export ONBUILD_TAG=`if [ "$TRAVIS_BRANCH" = "master" ]; then echo "onbuild"; else echo
    // $ echo "Main tag - $MAIN_TAG | Onbuild - $ONBUILD_TAG | Commit (shorten) - $COMMIT | Branch -
    ...
    // git clone git@github.com:gatsbyjs/gatsby-starter-hello-world.git
    // npm install -g gatsby-cli
