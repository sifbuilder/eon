...
(function (global, factory) {
  ...
  let muonnewton = function muonnewton(__mapper = {}) {

    let f = __mapper("props")()

    let abs = Math.abs, epsilon = 1e-6, halfPi = Math.PI/2, sqrt = Math.sqrt

    let asin  =Math.asin, atan2 = Math.atan2, cos = Math.cos, sin = Math.sin
    ...
    function cartesian(spherical) {
      let lambda = spherical[0], phi = spherical[1], cosPhi = cos(phi)
      ...
      function invert (x,y) {
        ...
        try {
          let solution = Newton.Solve(
            g => {
              let norm = g[0] * g[0] + g[1] * g[1] + g[2] * g[2]
 ...
 */
    let Newton = {version: "1.0.0"} // semver
    ...
    Newton.Solve = function(fs, res, opts={}){
      ...
      let n = Newton.Zero(fs, start, opts)
