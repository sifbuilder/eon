...
Q.prototype.blogs = function () {
  ...
  let blog = {header, content}
    ...
    let doit = function (data, context) {
      ...
      let act = __.args.inPattern
      let params = __.args.params
    ...
    let parseArgs = function (data = {}, context = {}) {
      ...
      if (optsq >= 1) {
          res.act = res.args[0] // pattern in first param
      }
      res.params = res.args.slice(1)
    ...
    function getHeline () {
      ...
      let mod = ' :: node do eons action debug'
      return `${def}${mod}`
    }
